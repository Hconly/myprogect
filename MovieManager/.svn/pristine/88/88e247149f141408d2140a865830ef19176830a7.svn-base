...
public class UI_Find extends JPanel {
	...
	public UI_Find() {
		...
		List<MovieDto> list = ms.findByShowName("m_name", uim.name);
	...
	for (int i = 0; i < list.size(); i++) {
		...
		panel.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				UI_Find.this.mid = ms.getbyName(panel.getName()).getMid();
